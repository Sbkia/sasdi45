...
MY_COLORS_NAMED = ['red', 'lime', 'DarkBlue', 'magenta',
                   'RebeccaPurple', 'green', 'maroon', 'orange']
MY_COLORS_RGB = [(255, 0, 0), (0, 255, 0), (0, 0, 139), (255, 0, 255),
                 (102, 51, 153), (0, 128, 0), (128, 0, 0), (255, 165, 0)]
...
def format_duration(duration_sec):
    ...
    time_days, remainder1 = divmod(float(duration_sec), 86400)
...
def read_parameters():
    """ INPUT sasdi directory,
        ...
        RETURN Sasdi version, max acceptable fps value (int), use or not audio output (1 or 0), and 2 tuples with lowercase and uppercase accepted video files extensions
    """
    ...
    if os.path.exists(params_dir):
        try:
            ...
            # return 100 as default fps, audio output, and 4 accepted file extensions
            return "?.?", 100, 0, tuple(['avi', 'mp4', 'wmv', 'asf']), tuple(['AVI', 'MP4', 'WMV', 'ASF'])
        else:
            return myparams["version"], int(myparams["max_fps"]), int(myparams["audio_output"]), tuple(myparams["extensions"]), tuple([x.upper() for x in myparams["extensions"]])
        ...
        return base_params["version"], int(base_params["max_fps"]), int(base_params["audio_output"]), tuple(base_params["extensions"]), tuple([x.upper() for x in base_params["extensions"]])
...
def check_list_videos(videopathdir, videolist, subdir_index):
    ...
    _, fps_limit, _, images_extensions_lower, extensions_upper = read_parameters()
    images_extensions = images_extensions_lower + extensions_upper
    list_videos = []
    # [0]:total number, [1]:to analyze, [2]:analyzed, [3]:fps or nb frames modified, [4]:corrupted
    stats_videos = [0, 0, 0, 0, 0]
        ...
        if current_file.endswith(images_extensions):
            ...
            try:
                ...
                video = cv2.VideoCapture(
                    os.path.join(videopathdir, current_file))
                nb_frames = video.get(cv2.CAP_PROP_FRAME_COUNT)
                fps = video.get(cv2.CAP_PROP_FPS)
            except cv2.error as cv2_error:  # Exclude empty video or with read error
                video.release()
                print(
                    f"Corrupted file, cannot read with opencv: {os.path.join(videopathdir, current_file)} -> {cv2_error}")
                ...
                if nb_frames < 10:
                    ...
                    try:
                        ...
                        print(
                            f"Corrupted number of frames={nb_frames}, and cannot read video header with ffprobe: {os.path.join(videopathdir, current_file)}")
                        stats_videos[4] += 1    # corrupted file
                        file_status = "C"
                    else:
                        ffprobe_nb_frames = round(
                            ffprobe_fps * ffprobe_duration)
                        if ffprobe_nb_frames > 10:
                            print(
                                f"Using number of frames read with ffprobe ({ffprobe_nb_frames}): {os.path.join(videopathdir, current_file)}")
                            file_status = "M"
                ...
                if modified_fps:        # if already given by user use modified fps
                    ...
                    try:
                        ...
                        print(
                            f"Corrupted fps={fps}, and cannot read video header with ffprobe: {os.path.join(videopathdir, current_file)}")
                        stats_videos[4] += 1    # corrupted file
                        file_status = "C"
                    else:
                        ...
                        if not modified_fps:
                            print(
                                f"Corrupted fps not corrected by user: {os.path.join(videopathdir, current_file)}")
                            stats_videos[4] += 1
                ...
                if file_status != "C":
                    ...
                    list_videos.append([current_file, round(fps, 2), round(
                        nb_frames), videopathdir, index_files, file_status, subdir_index])
                   # [0]:video filename, [1]:fps, [2]:nb_frames, [3]:pathname, [4]:file index, [5]:status ('A', '-', 'M', 'C'), [6]:subdir index (0 if not a serie)
                    ...
                    list_videos.append(
                        [current_file, 0, 0, videopathdir, index_files, 'C', subdir_index])
                    # [0]:video filename, [1]:fps, [2]:nb_frames, [3]:pathname, [4]:file index, [5]:status ('A', '-', 'M', 'C'), [6]:subdir index (0 if not a serie)
                index_files += 1
...
def create_serie_roi():
    ...
    _, _, _, images_extensions_lower, extensions_upper = read_parameters()
    images_extensions = images_extensions_lower + extensions_upper
    ...
    for index_subdir, current_subdir in enumerate(serie_subdir_names):
        ...
        try:
            ...
            print(
                f"No read/write permission for subdir {current_subdir}: {err}")
        else:
                ...
                if current_file.endswith(images_extensions):
                    ...
                    try:
                        video = cv2.VideoCapture(os.path.join(
                            main_fullpath, current_subdir, current_file))
                        video_width = int(video.get(cv2.CAP_PROP_FRAME_WIDTH))
                        video_height = int(
                            video.get(cv2.CAP_PROP_FRAME_HEIGHT))
                    except cv2.error as cv2_error:  # Exclude empty video or with read error
                        print(
                            f"ROI calculation: error reading {current_file}: {cv2_error}")
                        video.release()
            ...
            if (video_width == 0 or video_height == 0):      # no valid video in subdirectory
                ...
                roi_coord[index_subdir] = [
                    [(1, 1), (video_width-1, video_height-1)]]
...
def update_serie():
   ...
   # Get accepted video files extensions
    ...
    for subdir_index, current_sub_dir in enumerate(serie_subdir_names):
        ...
        list_all_files.sort()
        # Check all videos and get status statistics
        infos_videos, subdir_stats = check_list_videos(
            videopathdir, list_all_files, valid_subdir_index)
    ...
    if os.path.exists(os.path.join(sys.path[0], 'params', 'lastdir.txt')):
        ...
        with open(os.path.join(sys.path[0], 'params', 'lastdir.txt'), 'w') as file_writer:
            try:
                ...
                print(
                    f"lastdir.txt created with {sys.path[0]} as default path")
                return sys.path[0]
...
def user_guide(sasdi_version):
    ...
    message.append('')
    message.append(
        '  ADVICES FOR RECORDING VIDEOS BEFORE MOTION DETECTION BY VASD OR SASDI:')
    message.append(
        '      - To avoid observed size of mice changes upon position, videos shot from above are better than from aside,')
    message.append(
        '      - To enhance contrast, use a dark background with light-colored animal, and vice-versa,')
    message.append(
        '      - To avoid misplaced ROIs always keep the analyzed cage in the same place (add marks to repositioned precisely),')
    message.append(
        '      - For the same reason, be careful not to swap position of any elements when changing cages for example (bib, food dispenser, ...),')
    message.append(
        '      - For better animal detection, use a homogenous illumination, not blinking, and with minimal shades, during both days and nights,')
    ...
    message.append(
        '      - To properly detect seizure, the best configuration is 1 animal per zone (use a separator with holes to keep social interactions).')
    message.append(
        '      - To avoid errors, do not use space and special characters (except -) in all directories and file names.')
    message.append('')
    message.append(
        '  __________________________________________________________________________________________________________________________')
    message.append('')
    message.append(
        '  HOW TO USE SASDI INTERFACE FOR MOTION DETECTION ON MULTIPLE VIDEO FILES ?')
    message.append(
        '  __________________________________________________________________________________________________________________________')
    message.append('  STEP 1: SELECT tab')
    message.append('')
    message.append(
        '  BEWARE, analyzed directories should not contain csv files other than those generated by VASD or SASDI.')
    message.append('')
    message.append(
        '  TO VISUALISE RESULTS OF VIDEO FILES PREVIOUSLY ANALYSED WITH VASD OR SASDI :')
    message.append(
        '    * Select files as described below, ignore ROI selection and go directly to Step 3.')
    message.append('')
    message.append('  TO ANALYSE VIDEO FILE(S) :')
    message.append(
        '    * Select files and select ROI(s) to be used for analysis, as described below:')
    message.append('')
    message.append('  SELECTING VIDEO FILES :')
    message.append(
        '    * Select a full directory, a selection of video files, a serie, or keep the previous displayed selection.')
    ...
    message.append(
        '    * The list of video files is shown with their fps (frame per seconds) and durations, with the following indications:')
    message.append('        (-) Video file is not analyzed.')
    message.append(
        '        (A) Already analyzed video file (csv file present).')
    message.append(
        '    * You can click on video names in the list and unselect them one by one using the "Unselect a video" button.')
    message.append('    * The selection is kept if you close and reopen SASDI (beware if there are errors in some videos, display can be long, be patient and read infos in terminal).')
    message.append('')
    message.append('  SELECTING ROI (Region Of Interest) :')
    message.append('')
    message.append(
        '    * If not analyzing a serie, select at least one ROI corresponding to the zone(s) to be analyzed, or keep previous ROI(s) if corrects.')
    message.append(
        '          Click "Select new ROIs", the first valid image of the first video is shown, draw ROIs of same size for each animal zone,')
    message.append(
        '          press R to record each ROI, D to discard last ROI, and S to save selection.')
    message.append(
        '    * The list of selected ROIs is displayed, you can modify the list by clicking again on "Select New ROIs".')
    ...
    message.append('  ')
    message.append(
        '    * If analyzing a serie, by default the dimensions of each subserie ROI are set automatically ')
    message.append(
        '          to the size of the first video of each sub-directory,')
    message.append(
        '    * You can edit the ROIs used for each subdirectory by clicking on "Reselect serie ROIs",')
    message.append(
        '          you will then be prompted for EACH subdirectory to edit the ROI(s) as above, just press S to keep existing ROI.')
    message.append(
        '          The same set of ROIs will be used for all videos within each subdirectory.')
    message.append('')
    message.append(
        '  __________________________________________________________________________________________________________________________')
    ...
    message.append('')
    message.append(
        '    * You can adjust the number of threads to be use for analysis.')
    message.append(
        '          By default, it is set to 80% of total number of threads (to keep the computer running normally),')
    message.append(
        '          with a maximum of 1 thread per video file to analyze (more is useless and decreases the performance).')
    message.append('')
    message.append(
        '    * Click on "Start detection" button to start analyzing all selected files.')
    message.append(
        '    * Optionally, check "Allow reanalyze" to force analysis of all videos,')
    message.append(
        '          (already analyzed videos, with existing csv file in the same directory, are normally not analyzed again).')
    message.append('    * Wait for a message indicating end of analysis,')
    message.append(
        '          depending on computer SASDI analyzed roughly 100 fps (1 hour of video at 15 fps is analyzed in 9 minutes).')
    ...
    message.append('')
    message.append(
        '    * The BOTTOM GRAPH is showing the motion analysis in percent of max motion, color coded for each ROI.')
    message.append(
        '    * The ROI limits, color code, and index are displayed at the bottom right on top of an image from the current video.')
    message.append('')
    message.append(
        '    * In the center following information are shown, index of current video / total number of videos, name of current video file.')
    message.append(
        '    * The complete path to the current video file is indicated in the top.')
    message.append(
        '    * If a serie is visualized, the index of current subserie / total number of subseries is also shown.')
    message.append('')
    message.append(
        '    * The TOP GRAPH, is empty by default ("None" option is ticked).')
    message.append('')
    message.append(
        '    * Tick "Power" to display the sum of power spectra of all frequencies, calculated from the motion values.')
    message.append(
        '      This graph is very informative to spot seizures where a high power of motion is observed.')
    ...
    message.append('')
    message.append(
        '    * Tick "Audio", to display the audio stream, if available in the video file (only first audio stream is displayed).')
    message.append(
        '    * This can be usefull if your animal model emits a sound upon seizure.')
    ...
    message.append(
        '      FFmpeg is able to convert most video formats but it may not work if you are using an unusual format. ')
    ...
    message.append('')
    message.append(
        '    * Many BUTTONS are available to control the graphs display:')
    message.append(
        '      - MOTION Y AXIS SCALE:  Increase, decrease, or set automatically with buttons on the left or using keyboard (up and down arrows, ! keys),')
    message.append(
        '      - POWER or AUDIO Y AXIS SCALE:  Increase, decrease, or set automatically with buttons on the left or using keyboard (+ and -, * keys),')
    ...
    message.append(
        '        If time range is not full time, move display window backward or forward in time with buttons or using keyboard (B and F keys).')
    message.append(
        '      - If visualizing a SERIE, change display to previous or next subseries with buttons or using keyboard (P and N keys).')
    message.append(
        '        Note that "Previous serie" and "Next serie" buttons set active the first video of the displayed serie.')
    ...
    message.append(
        '        or go directly to desired video file by clicking on the slider.')
    message.append('')
    ...
    message.append(
        '      Default video player is VLC which enable to navigate easily and quickly in opened video.')
    message.append('      The second video player is coded with opencv (OCV), it can be used in case VLC cannot open the video and enables to see ROIs on top of video if needed, but display time is not real time.')
    message.append('')
    message.append('    * Switch to fullscreen using "f" key.')
    message.append(
        '    * QUIT results window with the "Quit" button or using keyboard (Q or q keys).')
    ...
    message.append('    * If you get a permission error when accessing video files (ex: you have selected a system directory) you will not be able to open Sasdi anymore, in sasdi/params directory delete all files except params.json')
    message.append('')
    message.append(
        '  __________________________________________________________________________________________________________________________')
    message.append('Dr Fabrice DUPRAT, duprat@ipmc.cnrs.fr, august 2021')
