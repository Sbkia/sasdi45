...
def one_video_analysis(arg):
    ...
    # Get max fps value
    _, fps_limit, _, _, _ = sf.read_parameters()
    ...
    for index_roi in range(0, nb_roi):
        # (y1-y2) * (x1-x2)
        roi_size.append(abs((int(roi_coord[index_roi][0][1]) - int(roi_coord[index_roi][1][1])) * (int(roi_coord[index_roi][0][0]) - int(roi_coord[index_roi][1][0]))))
